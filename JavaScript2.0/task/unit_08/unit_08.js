...
function t11() {
	...
	while (k < divs11.length) {
		strOut = strOut + divs11[k].innerHTML + "_";
		k++;
	}
	document.querySelector('.out-11').innerHTML = strOut;
}
...
function t12() {
	...
	let k = 0;
	while (k < divs12.length) {
		divs12[k].style.background = 'orange';
		k++;
	}
}
...
function t14() {
	let b = document.querySelectorAll('.i-14');
	let out = document.querySelector('.out-14');
	out.innerHTML = '';
	let k = 0;
	while (k <= b.length) {
		if (b[k].checked) {
			out.innerHTML += b[k].value;
			break;
		}
		k++;
	}
